...
def get_analytic_profile(class_instance, mass_list, z=0):
    Pe, r = [], []
    Rvir = []
    for m in mass_list:
        mvir = m*u.Msun/cu.littleh
        rvir = class_instance.get_rvirial(mvir, z)

        r_bins = np.logspace(np.log10(0.1), np.log10(1), 100)*rvir
        this_profile = class_instance.get_Pe(mvir, r_bins, z)
...
fit_par = ['gamma', 'alpha', 'M0', 'beta', 'eps1_0', 'eps2_0']
par_latex_names = ['\Gamma', '\\alpha', 'M_0', '\\beta', '\epsilon_1', '\epsilon_2']
...
best_chisq = likelihood(gd_samples.getMeans())
...
plt.title(f'Log likelihood={best_chisq:.3f}')
